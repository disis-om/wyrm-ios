...
The official iOS port of Wyrm. Phase 1 proved a UIKit + SwiftUI shell, SDL3 initialization, and a Vulkan clear frame presented through MoltenVK to a UIKit-owned `CAMetalLayer` in the Appetize simulator. Phase 2 is in progress: the original atlas and shaders are bundled, verified and uploaded to the GPU; build 15 attempts to present one original atlas tile. The real Wyrm snake is not rendered yet.
